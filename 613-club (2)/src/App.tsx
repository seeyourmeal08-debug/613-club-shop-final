  ...
  const handleDynamicStripeCheckout = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (data.url) {
        ...
        throw new Error("L'URL de paiement Stripe n'a pas été retournée par le serveur.");
      }
    } catch (error: any) {
      console.error(error);
                ...
                <div className="border-t border-neutral-100 bg-neutral-50 p-6 space-y-4">
                  <div className="space-y-1.5">
                    <div className="flex justify-between text-base">
                      <span className="font-headline font-extrabold text-neutral-950">TOTAL</span>
                      <span className="font-mono text-neutral-950 font-extrabold">{grandTotal.toFixed(2)} $ CAD</span>
                    </div>
                  </div>

                  <button
                    onClick={handleDynamicStripeCheckout}
                    disabled={isCheckoutLoading}
                    className="w-full bg-[#013220] hover:bg-[#01422b] active:scale-[0.98] disabled:opacity-75 disabled:pointer-events-none text-white py-4.5 rounded-md font-headline font-bold text-xs tracking-widest uppercase transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 cursor-pointer"
                  >
